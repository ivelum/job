...
def pipedrive_get_or_create_person(client, form_person):
    ...
    if len(resp['data']['items']):
        ...
        print(f'Person found: {result["id"]}')  # noqa: T201
        return result

    resp = client.persons.create_person(form_person)
    print(f'Person created {resp["data"]["id"]}')  # noqa: T201
    return resp['data']
...
def pipedrive_create_deals(client, job, person, data):
    ...
    print(f'Deals {resp["data"]["id"]} was created')  # noqa: T201
...
def lambda_handler(event, context):  # noqa: ARG001
    ...
    malformed_error = error_response('Wrong input: malformed body.')
    try:

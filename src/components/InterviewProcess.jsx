...
export default function InterviewProcess({ jobIsActive }) {
  return (
    <>
      <p>
        После заполнения{' '}
        {
          (jobIsActive)
            ? <Link to="form/">формы отклика</Link>
            : 'формы отклика'
        } вы увидите приглашение для записи на короткий ознакомительный звонок
        в Zoom, он занимает буквально 15 минут. Вы сможете выбрать удобное для
        вас время и немедленно забронировать его. На этом звонке можно обсудить
        любые интересующие вас вопросы по проектам или по вакансии, а также
        решить, каким образом продолжить общение - или пойти на техническое
...
InterviewProcess.propTypes = {
  jobIsActive: PropTypes.bool.isRequired,
};
